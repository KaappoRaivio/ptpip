...
public class MyPTP {
    public static void main(String[] args) throws IOException, InterruptedException {
        try (PTPSession session = new PTPSession("192.168.2.1", false)) {
...
//            System.out.println(transaction.getResponsePacket());
            System.out.println(PTPTransactionDataParser.parseTransactionData(session.sendCommand(new PTPRequestOut(new PTPPacketCmdRequest(PTPPacketCmdRequest.OpCodes.GET_DEVICE_PROP_VALUE, 2, List.of(0x5008)))).getTransactionData()).getField("value"));
            while (true) {
                session.idle(4000);
